...
void getree() 
{
	...
	TString EEG = gettreename(10); std::cout<<EEG<<endl;
	// branche names
	...
	TChain *allchaineeg_mc = new TChain(ALLCHAIN+"_MC");
	
	TChain *allchainrho_pre = new TChain(ALLCHAIN+"_Pre");
	TChain *allchainksl_pre = new TChain(ALLCHAIN+"_Pre");
	TChain *allchaineeg_pre = new TChain(ALLCHAIN+"_Pre");
   ...
   printf("# MCSUM_MC = %d \n", mcsumNb_MC);
   // cut check
   Int_t cutindex =  CUTTAG[0]+ CUTTAG[1]+ CUTTAG[2]+ CUTTAG[3];
   //cout<<cutindex<<endl;
   if (cutindex == 0) {
   	printf("=================Pre-selected===============\n");
   }	
   else if (cutindex == NbMode) {
   	printf("=====================Cut===================\n");
   }
   else {
   	printf("==============Cut incosistancy !!!============\n");
   	for (Int_t i=0;i<NbMode;i++) {
   		//cout<<CUTTAG[i]<<endl;
   		if (!CUTTAG[i]) {
   			std::cout<<modname[i]<<": No cuts applied!"<<endl;
   		}
   	}
   	printf("-----------------------------------------------\n");
   }
