...
void roofit_noeta1() {
   ...
   RooChebychev bkg4pdf("bkg4pdf","bkg4pdf",x,RooArgSet(a0,a1)) ;*/
	...
	RooRealVar f4("f4","",1-f0_init-f1_init-f2_init,0.,1.);
	RooRealVar bkgfrac("bkgfrac","bkgfrac",f1_init+f2_init,0.,1.);
	RooAddPdf bkgsum("bkgsum","Bkgsum",RooArgList(bkg1pdf,bkg2pdf,bkg4pdf),RooArgList(f1,f2,f4)) ;

	RooAddPdf model("model","model",RooArgList(sigpdf,bkgsum),sigfrac);
	//RooAddPdf model("model","model",RooArgList(sigpdf,bkg1pdf,bkg2pdf,bkg4pdf),RooArgList(sigfrac,f1,f2));
